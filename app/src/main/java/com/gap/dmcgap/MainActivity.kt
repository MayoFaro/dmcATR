...
fun MainScreen(
    ...
    LaunchedEffect(result, isManual) {
        result?.let {
            val message = when {
                isManual -> "Perfos actualisées avec les valeurs fournies."
                isFixedF1 -> "Optimisation avec contrainte S1max=${s1Items[s1Index]} kg"
                else -> "Optimisation sans contrainte"
            }
    ...
    // 6.0) récuperation des donnéees des pickers results pour le mode manuel
        ...
        val s2Items    = generateStringList(0..2050, step = 50)
        val s3Items    = generateStringList(0..750,  step = 50)


    // valeurs réelles issues des pickers de résultat (mode manuel)
    ...
    // Totaux qu’on va “injecter” dans PickerSection en mode manuel
    val totalManualPax   = manualPaxA + manualPaxB + manualPaxC
    val totalManualFret  = (manualF1 + manualF2 + manualF3).toInt()

    // 1) Crée un State qui pointe toujours vers la dernière version de ta lambda GO
    val currentOnGo by rememberUpdatedState(newValue = {
        // ce code sera exécuté plus tard, mais l’accès à paxAIndex, etc. y sera toujours frais
        Log.d("ON_GO", "isManual=$isManual — indices = [$paxAIndex, $paxBIndex, $paxCIndex]")
        viewModel.calculate(
            fuelKg      = stateMap[configs[0]]!!.value.toDouble(),
            tripFuelKg  = stateMap[configs[1]]!!.value.toDouble(),
            totalPax    = if (isManual) paxAItems[paxAIndex].toInt() +
                    paxBItems[paxBIndex].toInt() +
                    paxCItems[paxCIndex].toInt()
            else stateMap[configs[2]]!!.value,
            totalFretKg = if (isManual) manualF1 + s2Items[s2Index].toInt() + s3Items[s3Index].toInt()
            else stateMap[configs[3]]!!.value.toDouble(),
        ...
        Box(
            ...
            Box(
                ...
                PickerSection(
                    ...
                    onGo         = { currentOnGo()},
                    isManual = isManual,
                    totalManualPax    = totalManualPax,
                    totalManualFret   = totalManualFret,
                    result = result
                )
            }
            Column(
                ...
                result?.let { res ->
                    ...
                    val resultF1Index = s1Items
                        ...
                        .coerceAtLeast(0)
                    val wheelColorF1 = if (isFixedF1 || isManual) Color(0xFF6650a4) // violet manuel
                    else Color.Transparent           // en auto sans contrainte

                    // 2) Choisis l’index à afficher selon les 3 modes
                    val displayF1Index = if (!isManual && !isFixedF1) {
                        // auto  ET sans contrainte → résultat de l’optimiseur
                        resultF1Index
                    } else {
                        // auto avec contrainte OU manuel → ta valeur scrollée
                        s1Index
                    }
                    // 3) Ne recentre qu’en auto sans contrainte
                    val recenterS1 = if (!isManual && !isFixedF1) result else null
                    ...
                    AbsoluteResultPicker(
                        ...
                        isChecked              = isFixedF1,
                        gradientColor = wheelColorF1,
                        onCheckedChange        = {checked ->
                            isFixedF1 = checked
                            // quand on décochera en auto, on remet au résultat
                            if (!checked && !isManual) {
                                s1Index = resultF1Index
                            }
                        },
                        recenterKey            = recenterS1,
                        borderColor = Color(0xFF6650a4),
                        frictionMultiplier     = 0.8f     // par exemple, 20% plus d’inertie
                    )
                    //}
                ...
                result?.let { res ->
                    ...
                    val recenter = if (isManual) null else result
                    val displays2 = if (isManual)
                                        s2Index
                                    else generateStringList(0..2100, step = 50)
                                        .indexOf(res.distribution.fret2.toInt().toString())
                                        .coerceAtLeast(0)
                    val wheelColor = if (isManual) Color(0xFF6650a4) // violet manuel
                    else Color.Transparent

                    AbsoluteResultPaxPicker(
                        pickerKey = "S2_${s2Index}_${isManual}",
                        label = "S2",
                        items = generateStringList(0..2050, step = 50),
                        selectedIndex = displays2,
                        enabled = isManual,
                        gradientColor = wheelColor,
                    ...
                    AbsoluteResultPaxPicker(
                        ...
                        onIndexChange   = { newIndex ->
                            Log.d("PICKER_CHANGE", "PaxA callback → $newIndex")
                            paxAIndex = newIndex },
                        borderColor = Color(0xFF6650a4),
                        recenterKey =recenter
                    )
                ...
                result?.let { res ->
                    val displays3= if (isManual)
                        s3Index
                    else generateStringList(0..750, step = 50)
                        .indexOf(res.distribution.fret3.toInt().toString())
                        .coerceAtLeast(0)
                    val wheelColor = if (isManual) Color(0xFF6650a4) // violet manuel
                    else Color.Transparent
                    AbsoluteResultPaxPicker(
                        pickerKey = "S3_${s2Index}_${isManual}",
                        label = "S3",
                        items = generateStringList(0..750, step = 50),
                        selectedIndex = displays3,
                        enabled = isManual,
                        gradientColor = wheelColor,
                        borderColor = Color(0xFF6650a4),
