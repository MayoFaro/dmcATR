...
fun PickerSection(
    ...
    onGo         :() -> Unit,
    isManual: Boolean,
    totalManualPax  : Int = 0,
    totalManualFret : Int = 0,
    ...
    Column(
        ...
        configs.forEachIndexed { idx, cfg ->
            val textColor = when {
                isManual && idx == 2 -> Color.Red   // exemple : le 3ᵉ picker en rouge
                isManual  && idx == 3 -> Color.Red  // ou tous en gris
                else                          -> Color.Black
            }
            val overrideValue = when {
                isManual && idx == 2  -> totalManualPax      // pax total
                isManual && idx == 3 -> totalManualFret     // fret total
                else                 -> null
            }
            IconWithAnimatedPicker(
                iconRes        = cfg.iconRes,
                items          = remember(cfg) { generateStringList(cfg.range, cfg.step) },
                pickerValue    = overrideValue?:stateMap[cfg]!!.value,
                onPickerChange = { new ->
                    if (!isManual || idx < 2) {
                        // en mode auto on laisse changer tous
                        // en mode manuel, on n'autorise pas les 2 derniers
                        stateMap[cfg]!!.value = new
                    }
                },
